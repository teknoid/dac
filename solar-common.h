...
#define GSTATE_HEADER	"    pv ugrid dgrid today  tomo   sod   eod   soc   ttl  succ  foca avail  need  minu  surv flags"
struct _gstate {
	int pv;
	int produced;
	int consumed;
...
#define PSTATE_HEADER	"    pv  grid  akku   ac1   ac2   dc1   dc2   m1p   m2p   m3p   m4p   m1v   m2v   m3v   m4v   l1p   l2p   l3p   l1v   l2v   l3v     f  surp  load   rsl  ramp flags"
struct _pstate {
	int pv;
	int grid;
	int akku;
...
#define DSTATE_HEADER	" flags   inv  lock  ramp steal cload rload"
struct _dstate {
	int flags;
	int inv;
	int lock;
