...
static int id3_parse_v1_tag(FILE *f, struct ID3v2TagStruct *tag) {
	...
	if (buf[93] >= '0' && buf[93] <= '9' && buf[94] >= '0' && buf[94] <= '9' && buf[95] >= '0' && buf[95] <= '9' && buf[96] >= '0' && buf[96] <= '9') {
...
static int id3_parse_v2_tag(FILE *f, struct ID3v2TagStruct *tag) {
	...
	while (p < dlen && tagdata[p] != '\0') {
		...
		if (frameid[0] == '\0' || memcmp(frameid, "RVAD", 4) == 0 || memcmp(frameid, "RGAD", 4) == 0 || memcmp(frameid, "XRVA", 4) == 0) {
			p += flen;
			continue;
		}
		...
		fhskip = (fflags & FRAMEFL_GROUP ? 1 : 0) + (fflags & FRAMEFL_CRYPT ? 1 : 0) + (fflags & FRAMEFL_DLEN ? 4 : 0);
		if (fhskip > flen) {
			p += flen;
			continue;
		}
...
static int id3_search_tag(FILE *f, struct ID3v2TagStruct *tag) {
	...
	if (ret == 0) {
		...
		while (pos > 128) {
			...
			if (fread(buf, 1, 10, f) != 10)
				return M3G_ERR_READ;
			if (memcmp(buf, "3DI", 3) == 0 && buf[3] == 4 && ((buf[6] | buf[7] | buf[8] | buf[9]) & 0x80) == 0) {
				/* Parse ID3v2.4 tag */
...
static int id3_decode_rva2_frame(const struct ID3v2FrameStruct *frame, struct MP3GainTagInfo *info) {
	...
	if (p + 6 <= frame->len && (memcmp(frame->data + p, "track\0", 6) == 0 || memcmp(frame->data + p, "TRACK\0", 6) == 0)) {
		is_album_gain = 0;
		p += 6;
	} else if (p + 6 <= frame->len && (memcmp(frame->data + p, "album\0", 6) == 0 || memcmp(frame->data + p, "ALBUM\0", 6) == 0)) {
		is_album_gain = 1;
		p += 6;
	} else {
		return 0;
	...
	while (p + 4 <= frame->len) {
		...
		peak = 0;
		if (peakbits > 0)
			peak += (double) (frame->data[p + 4]);
		if (peakbits > 8)
			peak += (double) (frame->data[p + 5]) / 256.0;
		if (peakbits > 16)
			peak += (double) (frame->data[p + 6]) / 65536.0;
		if (peakbits > 0)
			peak = peak / (double) (1 << ((peakbits - 1) & 7));
		p += 4 + (peakbits + 7) / 8;
...
static int id3_decode_mp3gain_frame(const struct ID3v2FrameStruct *frame, struct MP3GainTagInfo *info) {
	...
	if (strcasecmp(buf, "MP3GAIN_UNDO") == 0) {
		...
		if (sscanf(buf + strlen(buf) + 1, "%d,%d,%c", &f1, &f2, &f3) == 3 && info != NULL) {
			info->haveUndo = 1;
			info->undoLeft = f1;
			info->undoRight = f2;
		...
		if (sscanf(buf + strlen(buf) + 1, "%d,%d", &f1, &f2) == 2 && info != NULL) {
