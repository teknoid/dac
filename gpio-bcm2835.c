...
static void test_timers() {
	...
	xlog("T0 %08u T1 %08u", timer[0], timer[1]);
	usleep(1000);
	xlog("T0 %08u T1 %08u", timer[0], timer[1]);
	usleep(1000);
	xlog("T0 %08u T1 %08u", timer[0], timer[1]);
...
static int init() {
	...
	if (timer == MAP_FAILED)
		return xerr("mmap timer failed: %s", strerror(errno));
